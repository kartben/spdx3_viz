...
export const accessorsMixin = {
  ...
  fileSnippetOverlays(spdxId) {
    if (!this.__fileSnippetOverlays) this.__fileSnippetOverlays = new Map();
    if (this.__fileSnippetOverlays.has(spdxId)) return this.__fileSnippetOverlays.get(spdxId);
    const overlays = this._buildFileSnippetOverlays(spdxId);
    this.__fileSnippetOverlays.set(spdxId, overlays);
    return overlays;
  },
  _buildFileSnippetOverlays(spdxId) {
    ...
    const overlays = [...groups.values()].sort(
  ...
  snippetLabel(snippet) {
    return snippetSymbolLabel(snippet) || snippetLineLabel(snippet) || 'snippet';
  },
  snippetLines(snippet) {
    return snippetLineLabel(snippet);
  },
  _resetSnippetMemos() {
    this.__snippetClaims = null;
    this.__snippetParents = null;
    this.__snippetChildren = null;
    this.__fileSnippetOverlays = null;
  },
  buildInputs(spdxId) {

...
describe('snippetSymbolLabel', () => {
  it('drops the file half of a west routine snippet name', () => {
    assert.equal(snippetSymbolLabel(routine), 'spi_flash_restore_cache');
  });

  it('falls back to the line span for a path:lines coverage name', () => {
    assert.equal(snippetSymbolLabel(lineRange), 'L112-114');
  });

  it('is empty for an unnamed or missing snippet with no range', () => {
    assert.equal(snippetSymbolLabel({ type: 'software_Snippet' }), '');
    assert.equal(snippetSymbolLabel(null), '');
  });

  it('keeps a leading @ rather than returning nothing', () => {
    assert.equal(snippetSymbolLabel({ name: '@odd' }), '@odd');
  });
});
...
function snippetGraph() {
  ...
  const map = new Map([...snippets, decl, macro].map((s) => [s.spdxId, s]));
  map.set(FILE, { spdxId: FILE, type: 'software_File', name: 'kernel/thread.c' });
  map.set(HEADER, { spdxId: HEADER, type: 'software_File', name: 'kernel/thread.h' });
  const relationships = [
...
describe('snippet overlay toggle', () => {
  it('closes a default-open overlay on the first click', () => {
    const app = Object.assign(Object.create(navigationMixin), { snippetOverlayOpen: {} });
    app.toggleSnippetOverlay('hasInput', true);
    assert.equal(app.snippetOverlayOpen.hasInput, false);
  });
});
...
function sourceGraph(pkg) {
  ...
  const map = new Map(files.map((f) => [f.spdxId, f]));
  map.set(pkg.spdxId, pkg);
  return [
    { packages: [pkg], elementMap: map },
    { containsIndex: new Map([[pkg.spdxId, ['f:a', 'f:b']]]) }
  ];
}

const SHA = '563a6fdf8e4ad2c7c97df74657fbbc4803266ffc';
const FORK_SHA = '107120db232f2ce699c6a871b12b844d85c11ab1';
...
describe('buildFileSourceIndex', () => {
  ...
  it('reads a purl from software_packageUrl when identifiers omit it', () => {
    const [parsed, idx] = sourceGraph({
      spdxId: 'p:z',
      name: 'zephyr-sources',
      software_downloadLocation: 'NOASSERTION',
      software_packageUrl: `pkg:github/zephyrproject-rtos/zephyr@${SHA}`
    });
  ...
  it('rewrites the bundled SPDX 3.1 sample onto the fork that has that commit', () => {
    const [parsed, idx] = sourceGraph({
      spdxId: 'p:z',
      name: 'zephyr-sources',
      software_downloadLocation: 'NOASSERTION',
      software_packageUrl: `pkg:github/zephyrproject-rtos/zephyr@${FORK_SHA}-off`
    });
    assert.match(
      buildFileSourceIndex(parsed, idx).get('f:a'),
      new RegExp(`kartben/zephyr/${FORK_SHA}/`)
    );
  });
});
